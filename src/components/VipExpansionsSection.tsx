            ...
            >
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    {isVipUnlocked ? (
                      ...
                      <span className="flex items-center gap-1 text-slate-500 text-xs font-bold bg-slate-100 px-2 py-0.5 rounded-full">
                        <Lock className="w-3 h-3 text-slate-400" /> ${bump.priceUsd} USD
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className="mt-6 space-y-2.5 pt-4 border-t border-slate-100">
                <div className="text-[11px] font-black uppercase tracking-wider text-slate-400">
                  {isEs ? 'Enlaces directos incluidos:' : 'Links diretos inclusos:'}
                </div>
                    ...
                    if (!isVipUnlocked) {
                      return (
                        <div
                          key={linkId}
                          onClick={onOpenUnlockModal}
                          className="p-3 bg-slate-50 hover:bg-amber-50/70 border border-slate-200 hover:border-amber-300 rounded-2xl flex items-center justify-between gap-3 cursor-pointer transition group/item"
                        >
                          <div className="flex items-center gap-2.5 min-w-0">
                            <div className="w-8 h-8 rounded-xl bg-slate-200 group-hover/item:bg-amber-200 text-slate-600 group-hover/item:text-amber-900 flex items-center justify-center shrink-0 transition">
                          ...
                          <span className="text-[10px] font-black px-2 py-1 rounded-lg bg-slate-200 group-hover/item:bg-amber-300 text-slate-700 group-hover/item:text-slate-950 shrink-0 transition">
                            {isEs ? 'Desbloquear' : 'Desbloquear'}
                          </span>
                        </div>
                      );
                    }
              ...
              {!isVipUnlocked && (
                <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between">
                  <span className="text-[11px] text-slate-500 font-medium">
                    {isEs ? 'Incluido en el Combo VIP ($9.90)' : 'Incluso no Combo VIP ($9.90)'}
                  </span>
                  <button
                    onClick={onOpenUnlockModal}
                    className="text-xs font-black text-pink-600 hover:text-pink-700 flex items-center gap-1 cursor-pointer"
                  >
                    <span>{isEs ? 'Desbloquear pack' : 'Desbloquear pack'}</span>
